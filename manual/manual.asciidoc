:doctitle:  The ALAN Adventure Language Reference Manual
:revnumber: 0.1.91-PreReleaseBeta7
:revdate:   March 03, 2019
:revremark: AsciiDoc conversion work
:doctype: book
:encoding: utf-8
:lang: en
// TOC Settings:
:toc: left
:toclevels: 5
// Sections Numbering:
:sectanchors:
:sectnums:
:sectnumlevels: 2
// Cross References:
:xrefstyle: short
:section-refsig: Sect.
// Misc Settings:
:experimental: true
:icons: font
:linkattrs: true

// =============================================================================
//                         Custom Attributes Definition
// =============================================================================
// Some attributes for custom substitution of multiple occurences of same text.

:alan-www: http://www.alanif.se/[www.alanif.se^]

// Path to shared images:
:assets-img: ../_assets/images/

// *****************************************************************************
// *                                                                           *
// *                            Document Preamble                              *
// *                                                                           *
// *****************************************************************************
// Show document info and AsciiDoc conversion version and date...

.About This Document
This document was converted to AsciiDoc by Tristano Ajmone; and it's still a work-in-progress draft.
The revision number and date refer to the editing status of the conversion work (ie, this document), not to the original manual from which the present document was derived.

This document was converted from the original _Alan Manual_ (in ODT format), version 3.0beta5 (August 9, 2018),
downloaded from:

* https://bitbucket.org/alanif/alan/src/fb81645/doc/manual/

The current document is being updated for the upcoming Alan Beta7 release, after which it should become the official replacement of the previous ODT/PDF edition.

This document is subjected to the https://bitbucket.org/alanif/alan/src/master/COPYING[Artistic License 2.0] terms.

// >>> Alan Logo >>>
:logo_width: width=500

ifeval::["{backend}" != "html5"]
:logo_width: scaledwidth=70%
endif::[]

image::alan-logo.png[Alan Logo,{logo_width},align="center"]
// <<< Alan Logo <<<


// ==============================================================================
//                             Include Book Chapters
// ==============================================================================

:leveloffset: +1

// 1. Introduction
// ===============
include::manual_01.asciidoc[]

// 2. Concepts
// ===========
include::manual_02.asciidoc[]

// 3. Lexical Definitions
// ======================
include::manual_03.asciidoc[]

// 4. Language Reference
// =====================
include::manual_04.asciidoc[]

// 5. Running An Adventure
// =======================
include::manual_05.asciidoc[]

// 6. Hints and Tips
// =================
include::manual_06.asciidoc[]

// 7. Adventure Construction
// =========================
include::manual_07.asciidoc[]

// Appendix A: How to Use the System
// =================================
include::manual_08.asciidoc[]

// Appendix B: A Sample Interaction
// ================================
include::manual_09.asciidoc[]

// Appendix C: Run-Time Messages
// =============================
include::manual_10.asciidoc[]

// Appendix D: Language Grammar
// ===========================
include::manual_11.asciidoc[]

// Appendix E: Predefined player words
// ===================================
include::manual_12.asciidoc[]

// Appendix F: Compiler Messages
// =============================
include::manual_13.asciidoc[]

// Appendix G: Localization
// ========================
include::manual_14.asciidoc[]

// Appendix H: Portability of Games
// ================================
include::manual_15.asciidoc[]

// Appendix I: Copying Conditions
// ==============================
include::manual_16.asciidoc[]

// Index
// =====
// We'll exclude it from HTML5 documents since that backend doesn't support it:
ifeval::["{backend}" != "html5"]

:sectnums!:

[index]
= Index

:sectnums:

endif::[]

// ...

:leveloffset: -1

// EOF //
