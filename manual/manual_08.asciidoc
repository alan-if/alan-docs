...
* `-import` add a directory to the search path for imported files (see <<Filenames>> for details on the `import` statement). This switch can be used multiple times, each adding a new directory
