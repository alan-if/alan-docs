...
It must of course be possible to change the value of an object's attributes.
...
// @TODO: In the image we could use different background colors for those classes
//        that can't be sub-classed or instanciated (ie: LITERAL, STRING, INTEGER)
//        and italicize the classes that are not an Alan keyword (ie: LITERAL, INTEGER).
//        IMO, it would make this a better image of reference for students.

[[fig-predefined-classes]]
.Relationships between the predefined classes.
image::predefined-classes.png[predefined classes, align="center"]
...
// @TODO: Here it should be specified that LITERAL and INTEGER don't have a corresponding
//        keyword in the language (unlike STRING, which can be used with ADD TO even
//        though not create instances or classes).
...
// @TODO: How should TRUE be styled? all caps? bold? italic?
...
// @FIXME: "In the following DESCRIPTIONS..." --- the text refers to these "descriptions"
...
// @NOTE: "to be their common PARENT" --- maybe ACENSTOR would be more appropriate here,
//        as it might apply to inderect descendants too?
//        Maybe the section on inheritance should introduce some basic OOP terminology too,
//        and a Glossary could be also added ad the end of the book!
...
In this case the contained instances can only be guaranteed to be their common parent, which might be a quite general class like `thing` or even `entity`.
...
// @FIXME: "describes how E.G. containment" Why "e.g."? One would expect a clear-cut
//         definition of transitivty here, not an example definition. The sentence
//         is not very cleary. Depending of what it's trying to say, it could be
//         "Transitivity describes, among the other things, how containment..."
//         or just "Transitivity describes how containment.."
...
// @NOTE: Maybe change to "Verbs are USUALLY associated with", because there are
...
// @FIXME: In this sentence some words must have been lost, because it doesn't
//         seem to make sense: "Instead need to reference the object the player..."
...
// @NOTE: The following considerations might be a bit premature in the text, and
//        confuse the reader, as he hasn't yet been told that actors are also
//        containers, and that the hero's inventory is just the objects contained
//        inside the hero. The sentence below takes a big leap when it says
//        "inside a container (the inventory)", as there is no immediate connection
//        between the 'hero' of the example and the "container" adn "inventory"
//        of the sentence --- it assumes the reader already posseses the knowledge
//        to connect all these dots, but he probably doesn't, at this stage!
...
// @NOTE: The following (rather precious) note seems a bit out of context here.
...
// @FIXME: The sentence "Therefore, the verb execution ..." doesn't make sense.
