...
The Alan compiler and interpreter will always allow multinational (((multinational characters))) 8-bit characters as input and the default messages are generated for 8-bit ((character sets)), internally representing national characters according to the ISO multinational character set (ISO-8859-1) requiring 8 bits.
...
Actors can be located, as can any `thing`.
Giving an actor an initial location inside a container is an error, but a `Locate` statement may put one there.
The behaviour of an actor inside a container is not yet completely well-defined and might change in future versions, so the compiler warns about it, and about containers declared to take ``actor``s.
...
(See <<words_identifiers_and_names>> for more details.)
...
The optional `Definite`, `Indefinite` and `Negative` ((``Article``s)) and ``Form``s can be used to define how an instance is printed in its definite, indefinite and negative forms.
...
You may override these via an `Add` statement.
================================================================================
...
// @tajmone: The following paragraph is a bit vague, it's not clear who's
//    performing execution of the `Article` statements (ARun, or the author
//    via the cdoe?) or the "normal printing of the instance".
//    If it was rephrased in a manner which makes it clear who's doing all this,
//    it would be much more easy to read an understand. E.g.:
//        "To print the indefinite ... form of an instance ... ARun simply ..."
...
// @ADD EXAMPLE! Thoni, could you provide a real example of the above mentioned
//    use cases, from Sweedish? Possibly, by adding English translation in the
//    side comments of the ALAN code.


Both `Article` and `Form` are inherited as a single property.
That means that an instance may override its inherited form using either of the two forms, regardless of how its parent defined the form.



==== Printing

// @FIXME: Too many 'forms' in the next paragraph!
//         The reader can't track which form refers to what.
//         In "various forms of the `Say` statement", does the word 'form' have
//         any relation to the forms as in ARITCLE/FORM, or is it just a way
//         to indicate the different uses of SAY?
//         I'm lost! It's a linguistic maze, a formless labyrinth where I feel
//         lost and uninformed!
...
By default the contents of the container will be listed unless it is empty or opaque.
If the default description has been overridden, the author will have to explicitly decide on how to handle the container aspect and its content.



==== LIMITS

The (((LIMITS, clause))) `Limits` clause of the `Container` property declaration makes it possible to define limitations on _what_ and _how much_ can be put in any given container (as a class or a instance).
These limitations are enforced by the interpreter during gameplay.
(((BNF, rules of, LIMITS)))
...
If any of these limits are exceeded when trying to put (i.e. `locate`) anything inside the container, the statements in the corresponding `Else` part will be executed and the player turn aborted.
In fact, these checks are performed because of the execution of a (((locating inside containers))) `Locate` statement (usually, but not limited to, as a result of the player issuing a command with the intent of placing something in a container).
This means that the execution of a sequence of statements can actually be interrupted by a `Locate` statement triggering limitations.

Each `Limits` specification requires either a limiting attribute or the special meta-attribute `Count`.
Multiple limitations can be defined on the same container.

In case of a (((LIMITS, limiting attribute))) limiting attribute, it must be a numeric attribute defined on the class taken by the container (by default, `object`), and its implications are that the sum of this attribute on all objects in the container cannot exceed the specified value.

As for the special attribute `Count`(((COUNT, in LIMITS))), it defines a limitation on the number of instances allowed in the container.
...
The `Count` limit considers all instances directly in the container.
This might differ from the number of instances listed e.g. if the container takes ``Thing``s (which are not automatically shown to the player).
E.g. if a container contains one instance of `Thing` and one instance of `Object` a listing will only mention the object, but the `Count` limit will still consider the count to be 2.

The calculation of a limiting attribute always sums the values of that attribute for all contained items recursively.
This includes the container itself and the added item, if it happens to be a container too.
In effect the calculation is transitive (see <<Transitivity>>).
As described in <<Containers Containing Containers>>, this process might involve instances that do not have the limiting attribute, in which case these are simply disregarded from the sum.

If an item is being located into a target container which also happens to be inside a container, the limits of that "`outer`" container are also checked.
This is repeated so that the limits of any containing containers are checked.
...
The `Extract` clause, including optional `Check` and `Does` clauses, allows the author to prohibit the extraction of an item from the container depending on some condition.
If the `Check` is present, it works the same way as for ``Verb``s (see <<Verb CHECKs>>), i.e. a `Check` without a guard expression will unconditionally prohibit extractions; a `Check` with an expression will evaluate that expression and, if false, execute its `Else` clause, and then abort the move.
The `Does` clause will be executed if the optional `Check` passes, or if there was no `Check`.
...
[WARNING]
================================================================================
Both limits and the `Extract` clause will, if triggered, interrupt the execution of the triggering `Locate` statement, and does not continue the execution.
Currently there is no way to programmatically check if this will happen.
The author needs to be aware of this effect and if necessary, take precautions.
================================================================================
...
quotes (i.e. a <<words_identifiers_and_names,quoted identifier>>) to prevent
...
When the description of a location already visited by the player is being omitted as a result of `Visits`, the <<msg_AGAIN,`AGAIN` run-time message>> will be shown after the location name.
...
See <<containment_classes_and_transitivity,_Containment, Classes and Transitivity_>> for some background information.
...
Since this may well end up with a set of classes with little in common, an aggregate over such a container looks for the aggregated attribute in each of them rather than in what they all have in common.
See <<Aggregates>>.
================================================================================
...
Any attribute referred to either in the aggregation itself or in the filters must be available in all instances that the aggregate can reach.
The usual way to ensure this is to filter out only instances of the relevant class, e.g. objects, using a class filter.

For `Sum`, `Min` and `Max` over a container there is one exception.
When the instances are selected with an `In` filter, it is enough that a single one of the classes that the container might contain has the attribute.
Those instances, and only those, are aggregated over; the rest are ignored.
This means that a container which takes a class that is itself a container does not prevent you from aggregating over an attribute of the class it takes.

If two unrelated classes that the container might contain each define an attribute of that name, it is still an error, since there is then no single class to aggregate over.

[NOTE]
================================================================================
This does not apply to `Count`, which has no attribute to aggregate over and therefore aggregates over every instance the filters let through.
An attribute used in a `Count` filter must be available in all of them.
================================================================================
...
For `Sum`, `Min` and `Max` the class narrowed down to by the aggregated attribute counts as known, so the filters may use attributes of that class as well.
