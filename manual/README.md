...
All work on the _Manual_ happens in the `master` branch. Every change is published automatically to the Alpha Manual, which therefore reflects the latest Alan [development snapshots]; the Beta Manual is published separately, when a new [Alan SDK] Beta is released. See [_Beta vs Alpha_](#beta-vs-alpha).

## Maintainers Check-list

Whenever a new Alan Beta version is released, before publishing the Beta Manual:

- `:revnumber:` in [`manual.asciidoc`][man] should name the release being published. The release workflow refuses to publish if it does not.
