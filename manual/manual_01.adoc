...
// @FIXME @tajmone: The English in the code example below should be improved!
//   * [ ] Fix: "In one end is" -> "At one end there's"
//   * [ ] Fix: "in the other"  -> "at the other"
//   * [ ] Fix: "From the smell of things"  -> rephrase!
//   * [ ] Check if example occurs again elsewhere!
...
image::{assets-img}predefined-classes.svg[predefined classes, align="center", scaledwidth="90%", width="90%"]
...
// @FIXME @tajmone: These code-templates are not rendered nicely due to the '<'
//      and '>' symbols being highlighted as operators ('o')!
//      I should check the Rouge CSS and see why operators are being shown in
//      cyan colour here!!!
...
The most important thing to remember is that you can extend the inheritance tree with new classes and that most features can be inherited along that tree and be overridden, both by intermediate classes in the inheritance chain and explicitly in the instance itself.
...
The `Sum`, `Min` and `Max` aggregates are an exception.
Rather than requiring the attribute on that common ancestor, they look for it among the classes that the container might contain, and aggregate over the instances of the class that has it, ignoring the rest.
See <<Aggregates>>.
Everywhere else, `Count` filters and `For Each` loops included, the common ancestor is still what you get.
...
Verbs are usually associated with a class or an instance.
...
So let's move the definition from the `kitchen_door` instance to the `door` class:
...
A reference to the exact object the player mentioned in his command is propagated in the parameter (see <<The SYNTAX>> and <<SYNTAX Definitions>> for a more thorough discussion).
Since this verb is now applicable to all `door`s, the attribute `closed` must also be available for all instances of the `door` classs.
We can do that by ensuring that the attribute exists in the class using a normal attribute declaration.
