...
This can be handy when you have source files written using a specific encoding.
Many terminals and editors today use the UTF-8 encoding but historically Alan only supported ISO8859-1 for non-ASCII characters.
Currently the Alan compiler will still default to `-charset iso` but in the near future UTF-8 will be the default encoding for Alan source files.
