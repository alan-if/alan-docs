...
Options (((options))) define things concerning the overall behaviour of the generated Alan adventure. As is implied they are optional and are only required if you need to change the value of an option from its default setting. An option follows the grammar
...
Assignment and comparisons between values requires the values to be compatible(((compatible types))). The three basic types (integer, string and Boolean) are only compatible with themselves.
...
// FIXME: In other places "`If`-statement" is used instead of "`If` statement",
//        which creates different Index entries. I should enforce one of the two!
...
Instances inheriting, directly or indirectly, from the predefined classes *thing*, *entity*, *object*, *location*, *actor* and *literal,* are subject to special semantics (((semantics of pre-defined classes))) and restrictions.
...
The base class `entity` (((entity, predefined class))) represents the lowest denominator of all instances. All other pre-defined classes inherit from `entity`. So adding a property to `entity` will add it to every instance.
...
It is possible to give an instance multiple names (((names, multiple))) by listing a number of name clauses. Each one will define adjectives and a noun as described above. The result is that the player can use any of the names to refer to the object. For example:
...
[NOTE]
================================================================================
The pre-defined class `entity` defines the (((Pronoun, predefined))) pronoun
...
The current location is set to the start location, and the current actor is the hero during the execution (((execution context, Initialize clause))) of all `Initialize` clauses.
...
An ((`Exit`)) has a list of identifiers, all of which are considered directional words. I.e. when any of those words is input by the player, he will be located at the location identified as the target of the exit. It is possible to customize the exit using a `Check`, (((Check, in exits))) that must be satisfied to allow passage through the exit, and statements (`Does`) that will be executed when the player passes through. The checks work as described in <<Verb Checks,_Verb Checks_>>.
...
* '```+```' -- (((indicator, multiple)))(((multiple indicator)))
  ...
  using ((*ALL*)) or concatenating a number of parameters using a conjunction
  like ((*AND*)), see <<Player Input>>).
...
[NOTE]
================================================================================
All ``Check``s (((Check(s), execution order))) for a `Verb` will always be run in the normal order regardless of any `Before`/`After`/`Only` qualifiers.
================================================================================
...
Each actor action and event execution is considered atomic (it can't be divided into smaller parts). All rule conditionals are evaluated (((Rule(s), executing))) after each actor (including the player) has acted (script step and player command respectively) and after each event has executed. In effect this will mean that a change in state will be detected almost immediately, if there is a rule for detecting that change.
...
There is a string (((string, containment operator))) ((containment operator)), ((`Contains`)), which can be used to test if a string contains another string. The test ignores any differences in character case. An example of an expression that is true is
