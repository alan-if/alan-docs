...
The current version of Alan does not properly support actors being inside containers or inside other actors, which could be a straight forward way to implement vehicles.
A `Locate` statement will put an actor in a container, but the compiler warns about it: the behaviour is not yet completely well-defined, may have strange side-effects, and might change in future versions.
